...
from .model import CLIP, CustomTextCLIP, convert_weights_to_lp, convert_to_custom_text_state_dict, resize_pos_embed, get_cast_dtype, resize_text_pos_embed, set_model_preprocess_cfg
...
def _get_hf_config(model_id: str, cache_dir: Optional[str] = None):
...
def get_tokenizer(model_name: str = "", context_length: Optional[int] = None, cache_dir: Optional[str] = None, **kwargs):
    if model_name.startswith(HF_HUB_PREFIX):
        model_name = model_name[len(HF_HUB_PREFIX) :]
        try:
            config = _get_hf_config(model_name, cache_dir=cache_dir)["model_cfg"]
        except Exception:
            tokenizer = HFTokenizer(model_name, context_length=context_length or DEFAULT_CONTEXT_LENGTH, cache_dir=cache_dir, **kwargs)
            return tokenizer
    else:
    ...
    if "hf_tokenizer_name" in text_config:
        tokenizer = HFTokenizer(text_config["hf_tokenizer_name"], context_length=context_length, cache_dir=cache_dir, **tokenizer_kwargs)
    else:
...
def load_state_dict(checkpoint_path: str, device="cpu", weights_only=True):
...
def load_checkpoint(model: Union[CLIP, CustomTextCLIP], checkpoint_path: str, strict: bool = True, weights_only: bool = True, device="cpu"):
...
def create_model(
    ...
    **model_kwargs,
    ...
    if pretrained:
        ...
        if pretrained_cfg:
            ...
            if pretrained_quick_gelu and not model_quick_gelu:
                warnings.warn(f"These pretrained weights were trained with QuickGELU activation but the model config does " f'not have that enabled. Consider using a model config with a "-quickgelu" suffix or enable with a flag.')
            elif not pretrained_quick_gelu and model_quick_gelu:
                warnings.warn(f"The pretrained weights were not trained with QuickGELU but this activation is enabled in the " f"model config, consider using a model config without QuickGELU or disable override flags.")
    ...
    if require_pretrained and not pretrained_loaded:
        ...
        raise RuntimeError(f"Pretrained weights were required for (model: {model_name}, pretrained: {pretrained}) but not loaded.")
...
def create_loss(args):
    if args.distill:
        return DistillClipLoss(local_loss=args.local_loss, gather_with_grad=args.gather_with_grad, cache_labels=True, rank=args.rank, world_size=args.world_size, use_horovod=args.horovod)
    elif "coca" in args.model.lower():
        return CoCaLoss(caption_loss_weight=args.coca_caption_loss_weight, clip_loss_weight=args.coca_contrastive_loss_weight, local_loss=args.local_loss, gather_with_grad=args.gather_with_grad, cache_labels=True, rank=args.rank, world_size=args.world_size, use_horovod=args.horovod)
    elif args.siglip:
        assert not args.horovod, "Horovod not currently supported for SigLip"
        return SigLipLoss(rank=args.rank, world_size=args.world_size)
    elif args.dhnnce_loss:
        return HardNegativeLoss(temperature=args.temperature_dhnnce, alpha=args.alpha_dhnnce, beta1=args.beta1_dhnnce, beta2=args.beta2_dhnnce, batch_size=args.batch_size)
    return ClipLoss(local_loss=args.local_loss, gather_with_grad=args.gather_with_grad, cache_labels=True, rank=args.rank, world_size=args.world_size, use_horovod=args.horovod)
...
def create_model_and_transforms(
    ...
    **model_kwargs,
    ...
    force_preprocess_cfg = merge_preprocess_kwargs({}, mean=image_mean, std=image_std, interpolation=image_interpolation, resize_mode=image_resize_mode)
